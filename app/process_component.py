...
@router.post("/api/arrayskips")
def postarrayskips(data: dict):
    if 'Startfrom' in data:
        print("Update startfrom")
        backend_variables.startfroms = data['Startfrom']
    if 'Ignore' in data: 
        print("Update ignore")
        backend_variables.ignores = data['Ignore']
    return {"msg":"ok"}


@router.get("/api/start_process")
async def start_process():
    if not backend_variables.websocket_payload['door'] or not backend_variables.websocket_payload['null_cut']:
        print('Missing nullcut or door - cannot start process')
        return {"msg": "Missing nullcut or door"}
    
    if not backend_variables.websocket_payload['mat_begin']:
        print('Missing material begin cannot start process')
        return {"msg":"Missing material"}
...
async def async_start_process():
    if not backend_variables.websocket_payload['null_cut']:
